...
for (player in names(player_info)) {
  points_per_game[[player]] <- 
    player_info[[player]][[4]] / player_info[[player]][[3]]
}
points_per_game
```
...
```{r}
points_per_game_round <- lapply(points_per_game, function(x) {round(x, digits = 2)})
...
mat_rand <- apply(mat_rand, 1:2, function(x) rounding(x))
mat_rand
```
...
```{r}
avg_temps <- apply(max_temps[-1:-2], 2, mean)
avg_temps
```
...
```{r}
medians_co <- apply(colorado_temps[-1:-2], 1, median)

max_med <- max(medians_co)
max_index <- which.max(medians_co)

min_med <- min(medians_co)
min_index <- which.min(medians_co)
